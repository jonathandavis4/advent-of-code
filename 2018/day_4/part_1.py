...
sleepiest_guard = sorted(times_slept_data, key=lambda x: x[1], reverse=True)[0][0]

print int(max(guards[sleepiest_guard].items(), key=lambda x: x[1])[0]) * int(sleepiest_guard)
